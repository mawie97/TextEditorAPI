...
public final class CommandFactory {
    ...
    public static Command from(CommandRequest r) {
        if (r == null) throw new IllegalArgumentException("request cannot be null");
        if (r.type() == null) throw new IllegalArgumentException("type cannot be null");
        ...
        return switch (r.type()) {
            ...
            default -> throw new IllegalArgumentException("Unknown command type: " + r.type());
        };
    }

    private static String requireText(CommandRequest r) {
        if (r.text() == null) throw new IllegalArgumentException("text is required for INSERT");
        return r.text();
    }

    private static int requirePos(CommandRequest r) {
        if (r.pos() == null) throw new IllegalArgumentException("pos is required for this command");
        return r.pos();
    }
}